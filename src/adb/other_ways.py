...
class QQPraise():  
    ...
    def prepare(self):  
        ...
        self.pattern = "bounds=\"(\d1,4),(\d1,4)(\d1,4),(\d1,4)[^>]*><node[^>]*text=\"联系人\" resource-id=\"com\.tencent\.mobileqq:id/name\""  
        contact_widget=self.parse_UI(self.contact_UI_content)  
        if contact_widget:  
            os.system("adb shell input tap %s %s" % (self.x, self.y))  
        # 选择好友列表  
        self.load_layout_xml("contacts_list.xml",1)  
        points=re.search("<node[^>]*text=\"好友\"[^>]*bounds=\"(\d1,4),(\d1,4)(\d1,4),(\d1,4)",self.contact_UI_content).groups()  
    ...
    def load_layout_xml(self, xml_name,option):  
        ...
        os.system("adb shell uiautomator dump /sdcard/%s" % xml_name)  
        os.system("adb pull /sdcard/%s ./%s" % (xml_name, xml_name))  
    ...
    def find_type_of_selected_widget(self):  
        self.load_layout_xml("contacts_list.xml",1)  
        self.pattern = "<node[^>]*(id/group|LinearLayout)[^>]*selected=\"true\"[^>]*bounds=\"(\d1,4),(\d1,4)(\d1,4),(\d1,4)\"[^>]*>"  
        contact_widget=self.parse_UI(self.contact_UI_content)  
        if contact_widget:  
    ...
    def expand_group(self):  
        ...
        self.pattern = "\"><node[^>]*checked=\"([^\"]*)\"" % coordinate  
    ...
    def praise_and_return(self):  
        ...
        # 找点赞位置  
        self.pattern="<node [^<]*点击可赞[^>]* bounds=\"(\d1,4),(\d1,4)(\d1,4),(\d1,4).*"  
